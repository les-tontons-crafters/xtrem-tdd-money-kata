...
```text
Bank is not setup -> returns a failure : No bank defined
Bank is setup with EUR as pivot, evaluate an empty Portfolio -> ???
Bank is setup with EUR as pivot without rates, trying to evaluate the Portfolio in USD -> Missing exchange rate(s): "USD","KRW"  
Bank is setup with EUR as pivot with rates for USD and KRW, evaluate a Portfolio(3992 EUR, 4567 USD, -45898988 KRW, 8967.89 USD) in USD -> return a success with ???
```
