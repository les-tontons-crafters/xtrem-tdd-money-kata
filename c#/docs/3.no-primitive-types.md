...
We have to come back in the green state by plug-in the new `Add` method to the. existing one
...
We are back in a safe state with our green test. But it's not over, we have to go through the third state of TDD: Refactoring!
...
Still, others are failing. 
We need to adapt assertions accordingly
