...
```scala
  test("round trip in error") {
    val originalAmount = -1.7462583255741694e-5
    val roundTripMoney = roundTripConvert(originalAmount, EUR, KRW)
...
```scala
  test("round trip in error") {
    val originalAmount = -1.7462583255741694e-5
    val roundTripAmount = roundTripConvert(originalAmount, EUR, KRW).value
...
class BankProperties extends AnyFunSuite with Checkers with EitherValues {
  ...
  test("round trip in error") {
    val originalAmount = -1.7462583255741694e-5
    val roundTripAmount =
      roundTripConvert(originalAmount, EUR, KRW).value.amount
