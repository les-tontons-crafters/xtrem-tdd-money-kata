...
:large_red_circle: Let's see what it looks like when we group `amount` and `currency` into a `Money` type: 
...
:large_green_circle: We generate the `Money` type from the test and adapt the `times` method from the test as well
...
:large_blue_circle: We can refactor the `Money` to return `Money` objects from methods and no more `double`
